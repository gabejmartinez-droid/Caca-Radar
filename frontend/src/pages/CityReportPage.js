...
export default function CityReportPage() {
    ...
    const fetchSummary = async () => {
      ...
      try {
        const endpoint = canUseSearch
          ? `${API}/city-reports?city=${encodeURIComponent(requestedCity)}`
          : `${API}/city-reports/share?city=${encodeURIComponent(requestedCity)}`;
  ...
  const handleShare = async () => {
    ...
    const shareUrl = `${HOSTED_WEB_URL}/city-report?city=${encodeURIComponent(summary.city)}`;
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: formatTranslation(t, "cityReportUi.shareTitle", { city: summary.city }),
          text: shareText,
          url: shareUrl,
        });
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA]" data-testid="city-report-page">
      ...
      <div className="max-w-lg mx-auto px-4 pb-20">
        ...
        {canUseSearch && (
          <div className="bg-white rounded-2xl border border-[#8D99AE]/10 shadow-sm p-4 mb-4">
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-2xl border border-[#8D99AE]/10 shadow-sm p-5">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <h2 className="text-2xl font-black text-[#2B2D42] truncate">{summary.city}</h2>
            ...
            <div>
              <div className="flex items-center gap-2 mb-2 text-[#2B2D42]">
                ...
                <h3 className="font-bold">{t("cityReportUi.mapTitle")}</h3>
              </div>
