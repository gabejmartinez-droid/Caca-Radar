...
async def send_municipality_admin_notification(details: dict, event_type: str = "created") -> dict:
    ...
    apple_product_id = escape(str(details.get("apple_product_id") or ""))
    google_product_id = escape(str(details.get("google_product_id") or ""))
    store = escape(str(details.get("store") or ""))
    ...
    <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 560px; margin: 0 auto; padding: 32px;">
      ...
      <div style="background: #F8F9FA; border-radius: 16px; padding: 20px; margin-bottom: 20px;">
        ...
        {"<p style='color: #2B2D42; font-size: 14px; margin: 6px 0;'><strong>Tienda:</strong> " + store + "</p>" if store else ""}
        <p style="color: #2B2D42; font-size: 14px; margin: 6px 0;"><strong>Product ID:</strong> {product_id}</p>
        {"<p style='color: #2B2D42; font-size: 14px; margin: 6px 0;'><strong>Apple Product ID:</strong> " + apple_product_id + "</p>" if apple_product_id else ""}
        {"<p style='color: #2B2D42; font-size: 14px; margin: 6px 0;'><strong>Google Product ID:</strong> " + google_product_id + "</p>" if google_product_id else ""}
